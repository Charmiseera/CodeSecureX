...
async def generate_pdf_report(scan_id: str) -> ReportResponse:
    """
    ...
    report = Report(scan_id=scan.id, pdf_filename=filename)
    await report.insert()
...
def _build_pdf(
    ...
    story.append(Paragraph("🔒 SecureCodeAI — Security Scan Report", title_style))
    ...
    story.append(Paragraph(
        "Generated by <b>SecureCodeAI</b> — LLM-Based Vulnerability Scanner", sub_style))
